/**
 * WalletService — Root level wrapper around @creit.tech/stellar-wallets-kit v2 static API.
 *
 * Core functionality:
 *  1. StellarWalletsKit.init({ modules, network })  — initialize supported wallet modules
 *  2. StellarWalletsKit.setWallet(id)               — select active module (Freighter, xBull)
 *  3. StellarWalletsKit.fetchAddress()              — prompt user extension & fetch public key
 *  4. StellarWalletsKit.getAddress()                — retrieve current connected public key
 *  5. StellarWalletsKit.signTransaction(xdr, opts)  — request wallet to sign transaction XDR
 *  6. StellarWalletsKit.disconnect()               — clear active wallet session
 */
...
export class WalletService {
  ...
  /**
   * Initialise StellarWalletsKit lazily on demand.
   */
  ...
  /**
   * Connect a specific wallet module by ID (Freighter or xBull). Prompts user.
   */
  ...
  /**
   * Sign a transaction XDR envelope using the connected wallet.
   */
  ...
  /**
   * Disconnect / clear the active wallet session.
   */
  disconnect() {
    this.selectedModuleId = null;
    try {
